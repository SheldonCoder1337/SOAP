<template>
  ...
  <div class="graph-container layout-container" v-else>
    <HeaderComponent
      :title="$t('graphdb_header_title')"
      :description="$t(`${graphInfo?.database_name || ''} - with ${graphInfo?.entity_count || 0} 'entity'，${graphInfo?.relationship_count || 0} 'relation'`)"
    >
      <template #actions>
        <div class="status-wrapper">
          <div class="status-indicator" :class="graphStatusClass"></div>
        </div>
        <a-button type="primary" @click="state.showModal = true" ><UploadOutlined/> {{$t('upload')}}</a-button>
      </template>
    </HeaderComponent>
    ...
    <div class="actions">
      ...
      <div class="actions-right">
        ...
        <a-button @click="loadSampleNodes" :loading="state.fetching">{{$t('fetch_entity')}}</a-button>
      </div>
    </div>
    ...
    <a-modal
      :open="state.showModal" title="upload files"
      @ok="addDocumentByFile"
      @cancel="() => state.showModal = false"
      ok-text="Add into graphDB" cancel-text="cancel"
      ...
      <div class="upload">
        <a-upload-dragger
          ...
          @drop="handleDrop"
        >
          <p class="ant-upload-text">{{$t('upload_msg')}}</p>
