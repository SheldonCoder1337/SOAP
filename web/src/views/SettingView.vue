<template>
  <div class="">
    <HeaderComponent 
      :title="$t('settings')" class="setting-header">
      <template #description>
        <p>{{$t('settings_header_desc')}}</p>
      </template>
      <template #actions>
        <a-button type="primary" v-if="isNeedRestart" @click="sendRestart">
          <ReloadOutlined />{{$t('reload_msg')}}
        </a-button>
        <a-button v-else @click="sendRestart">
          <ReloadOutlined />{{$t('reload_server')}}
        </a-button>
      </template>
    </HeaderComponent>
    <div class="setting-container layout-container">
      <div class="sider">
        <a-button type="text" :class="{ activesec: state.section === 'base'}" @click="state.section='base'" :icon="h(SettingOutlined)">{{$t('basic_config')}} </a-button>
        <a-button type="text" :class="{ activesec: state.section === 'model'}" @click="state.section='model'" :icon="h(CodeOutlined)">{{$t('model_config_path')}}</a-button>
        <a-button type="text" :class="{ activesec: state.section === 'path'}" @click="state.section='path'" :icon="h(FolderOutlined)">{{$t('path_config')}}</a-button>
      </div>
      <div class="setting" v-if="state.section === 'base'">
        <h3>{{$t('base_model_config')}}</h3>
        <div class="section">
          <div class="card">
      ...
      <div class="setting" v-if="state.section === 'model'">
        <h3>{{$t('model_config')}}</h3>
        <p>{{$t('model_config_msg')}}</p>
        <div class="model-provider-card">
          <div class="card-header">
            <h3>{{$t('self_define_model')}}</h3>
          </div>
          <div class="card-body">
            <div
            ...
            >
              <div class="card-models__header">
                ...
                <div class="action">
                  <a-popconfirm
                    :title="$t('delete_model_msg')"
                    @confirm="handleDeleteCustomModel(item.custom_id)"
                    :okText="$t('confirm_delete_model')"
                    :cancelText="$t('cancel')"
                    ok-type="danger"
            ...
            <div class="card-models custom-model" @click="customModel.visible=true">
              <div class="card-models__header">
                <div class="name"> + {{$t('add_models')}}</div>
              </div>
              <div class="api_base">{{$t('add_openai_model')}}</div>
              <a-modal
                ...
                :ok-type="'primary'"
              >
                <p>{{$t('add_openai_model')}}</p>
                <a-form :model="customModel" layout="vertical" >
                  <a-form-item :label="$t('model_name')" name="name" :rules="$t([{ required: true, message: 'pls_enter_model_name' }])">
                    <a-input v-model:value="customModel.name" />
                  </a-form-item>
                  <a-form-item label="API Base" name="api_base" :rules="$t([{ required: true, message: 'pls_enter_model_apikey' }])">
        ...
        <div class="model-provider-card" v-for="(item, key) in notModelKeys" :key="key">
          <div class="card-header">
            <h3>{{ modelNames[item].name }}</h3>
            <a :href="modelNames[item].url" target="_blank">{{$t('details')}}</a>
            <div class="missing-keys">
              {{$t('need_config"')}} <span v-for="(key, idx) in modelNames[item].env" :key="idx">{{ key }}</span>
            </div>
          </div>
        </div>
      </div>
